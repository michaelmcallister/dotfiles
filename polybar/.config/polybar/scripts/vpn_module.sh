...
# move this above the first if statement if something breaks
		...
		if hash geoiplookup 2>/dev/null; then
			...
			echo "%{u$COLOR_CONNECTED}$icon_polybar $city $country%{+u}"
		else
			echo "%{u$COLOR_CONNECTED}$icon_polybar $ip_address%{+u}"
		fi
	elif [ "$VPN_STATUS" = "$CONNECTING" ]; then
		echo "%{u$COLOR_CONNECTING}$icon_polybar Connecting...%{+u}"
	else
		echo "%{u$COLOR_DISCONNECTED}$icon_polybar No VPN%{+u}"
	fi
}
